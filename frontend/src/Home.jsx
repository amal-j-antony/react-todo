...
import { addTaskAPI, deleteTaskAPI, getTasksAPI, updateTaskAPI } from './services/allAPI'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
function Home() {
    ...
    const putEdit = async (editId, editBody) => {
        try {
            const result = updateTaskAPI(editId, editBody)
            console.log(result);
            ...
            <main className='w-full flex flex-col justify-center items-center'>
                ...
                <section className='container flex justify-center '>
                    <div className='flex flex-col gap-5'>
                        ...
                        <input onChange={(e) => setTask({ ...task, title: e.target.value })} className='border border-slate-500 py-2 px-5 rounded-2xl' type="text" placeholder='Add task title' />
                        <input onChange={(e) => setTask({ ...task, description: e.target.value })} className='border border-slate-500 py-2 px-5 rounded-2xl' type="text" placeholder='Add task description' />
                        {editId &&
                ...
                <section className='container gap-10 flex flex-col items-center border border-slate-500 rounded-2xl min-h-[50vh] mt-10'>
                    ...
                    <div className='flex gap-10 justify-start px-20 container'>
                        {
